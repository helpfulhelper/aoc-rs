...
fn gen_alpha() -> Vec<char> {
    ...
    // let lower_alpha = (b'a'..b'z').map(|c| c as char).collect();
    // let upper_alpha = (b'A'..b'Z').map(|c| c as char).collect();
}
